...
## Change 5: Input Feedback UI

> **Note:** This section addresses the user experience gap where the application appears "Idle" while it is still typing text from the background queue.

### 5.1 Problem Analysis: Hidden Activity

#### Symptoms

- User finishes speaking.
- App status changes to "Idle" immediately.
- Text is still being typed into the active window for several seconds.
- User might try to speak again or close the app, thinking it's done, leading to confusion or interrupted typing.

#### Root Cause: Decoupled UI State

With the implementation of **Change 4 (Input Queue)**, the `typewrite` method returns immediately. The main application considers the "transcription task" complete and resets the UI to "Idle", unaware that the background worker is still busy typing.

### 5.2 Proposed Solution: "Typing" State

We need to synchronize the UI with the background input worker.

#### Architecture

1. **Signals**: The `InputSimulator` should emit `typingStarted` and `typingFinished` signals.
2. **State Management**: The `viappApp` should listen to these signals and update the `MainWindow` state.
3. **Visual Feedback**: The `MainWindow` should display a "Typing..." status and potentially a distinct icon/color.

### 5.3 Implementation Plan

#### Target Files

- `src/input_simulation.py`: Inherit from `QObject`, add signals.
- `src/main.py`: Connect signals to state management.
- `src/ui/main_window.py`: Add visual handling for "typing" state.
...
```python
    def initialize_components(self):
        # ...
        self.input_simulator.typingStarted.connect(lambda: self.stateChanged.emit("typing"))
        self.input_simulator.typingFinished.connect(lambda: self.stateChanged.emit("idle"))
```

### 5.4 Benefits

- **Transparency**: User knows exactly when the app is busy working.
- **Better UX**: Prevents user from interrupting the typing process accidentally.
